...
public class Lower extends AbstractOpIneq {
    ...
    protected void codeGenBoolIMA(DecacCompiler compiler, BoolCtrlFlow flow) {
    	...
    	if(flow.getBranchCond())
    		compiler.addInstruction(new BLT(flow.getBranchLabel()));
    	else
    		compiler.addInstruction(new BGE(flow.getBranchLabel()));    	
    }
    ...
    protected void codeGenBoolGBA(DecacCompiler compiler, BoolCtrlFlow flow) {
        ...
        if(flow.getBranchCond())
    		compiler.addInstruction(new GB(flow.getBranchLabel(), GCondition.LT));
    	else
            compiler.addInstruction(new GB(flow.getBranchLabel(), GCondition.GE));
